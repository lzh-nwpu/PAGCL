...
if __name__ == '__main__':
    ...
    parser.add_argument('--perturb', action="store_false")
    parser.add_argument('--attack_method', type=str, default='CLGA')
    parser.add_argument('--attack_rate', type=float, default=0.01)
    default_param = {
        'learning_rate': 0.01,
        'num_hidden': 256,
        'num_proj_hidden': 32,
    ...
    if args.perturb:
        try:
            ...
            perturbed_features = pkl.load(
                open('poisoned_features/%s_%s_features.pkl' % (args.dataset, args.attack_method), 'rb')).to(device)
        except:
            ...
            perturbed_features = torch.load(
                'poisoned_features/%s_%s_adj.pkl' % (args.dataset, args.attack_method),
                map_location=device)
        data.edge_index = perturbed_adj.nonzero().T

    data = data.to(device)
    #生成插入节点的y值
    # y_insert = torch.full([8,], 100)
    y_insert = torch.randperm(2)
    y_insert = torch.cat([y_insert, y_insert, y_insert, y_insert])
    y_new = torch.cat([data.y, y_insert.to(device), y_insert.to(device), y_insert.to(device), y_insert.to(device), y_insert.to(device), y_insert.to(device), y_insert.to(device), torch.tensor([0,2,1,4]).to(device)])
    data = Data(edge_index=data.edge_index, x=perturbed_features, y=y_new).to(device)

    #给polblogs数据集划分mask
    split = generate_split(dataset[0].num_nodes, train_ratio=0.1, val_ratio=0.1)
    if args.dataset == 'PolBlogs':
        dataset.data.train_mask = split[0]
        dataset.data.val_mask = split[1]
        dataset.data.test_mask = split[2]
    #重新划分数据集
    # train_mask, val_mask, test_mask = generate_grb_split(data)
    insert_false = torch.full((60,), False).to(device)
    data.train_mask = torch.cat([dataset.data.train_mask.to(device), insert_false])
    data.val_mask = torch.cat([dataset.data.val_mask.to(device), insert_false])
    data.test_mask = torch.cat([dataset.data.test_mask.to(device), insert_false])
