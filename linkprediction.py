...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_method', type=str, default='PAGCL')
    ...
    y_new = torch.cat([data.y, y_insert.to(device), y_insert.to(device), y_insert.to(device), y_insert.to(device),
                       y_insert.to(device), y_insert.to(device), y_insert.to(device),
                       torch.tensor([0, 2, 1, 4]).to(device)])
