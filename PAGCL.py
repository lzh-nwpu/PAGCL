...
from differentiable_models.model_pyg import GCN_GIA
...
class Metacl(Module):
    def __init__(self, args, dataset, param, device):
        ...
        # self.new_data = dataset.data.to(device)
    ...
    def attack(self):
        ...
        print('Begin gia.....')
        # 设置GIA需要的参数
        gia_lr = 0.01  # learning_rate
        gia_epoch = 500
        gia_n_inject_max = 60
        gia_n_edge_max = 20
        gia_feat_lim_min = -1.0
        gia_feat_lim_max = 1.0
        gia_early_stop = 200
        gia_disguise_coe = 1.0
        gia_hinge = False

        # 进循环之前先进行GIA
        attacker = GIA(epsilon=gia_lr,
                       n_epoch=gia_epoch,
                       n_inject_max=gia_n_inject_max,
                       n_edge_max=gia_n_edge_max,
                       feat_lim_min=gia_feat_lim_min,
                       feat_lim_max=gia_feat_lim_max,
                       device=device,
                       early_stop=gia_early_stop,
                       disguise_coe=gia_disguise_coe,
                       hinge=gia_hinge)

        # 设置attacker.attack方法需要的参数
        # model
        gia_hidden_channels = 64
        ...
        model_gcn = GCN_GIA(data.num_features, gia_hidden_channels,
                            num_classes, num_layers,
                            gia_dropout, layer_norm_first=layer_norm_first,
                            use_ln=use_ln)

        attack_labels = None

        target_idx = torch.arange(0, data.x.shape[0], 1).to(device)

        adj_sparsetensor = SparseTensor(row=data.edge_index[0], col=data.edge_index[1],
                                        sparse_sizes=(data.num_nodes, data.num_nodes)).to(device)

        adj_attack, features_attack = attacker.attack(model=model_gcn,
        ...
        # 构造新的特征矩阵
        x_new = torch.cat([data.x.to(device), features_attack], dim=0) if features_attack != None else data.x
        ...
        self.data = new_data
        poisoned_features = self.data.x.to(device)
        pkl.dump(poisoned_features.to(torch.device('cpu')),
                 open('poisoned_features/%s_PAGCL_features.pkl' % args.dataset, 'wb'))
        print('---perturbed features matrix saved---')
        #end gia
        ...
        while len(perturbed_edges) < int(0.10 * num_total_edges):
            if len(perturbed_edges) == int(0.01 * num_total_edges) or \
                ...
                pkl.dump(output_adj.to(torch.device('cpu')), open('poisoned_adj/%s_PAGCL_0.010000_adj.pkl' % args.dataset, 'wb'))
            ...
            if len(perturbed_edges) == int(0.05 * num_total_edges) or \
                ...
                output_adj = adj.to(device)
                pkl.dump(output_adj.to(torch.device('cpu')), open('poisoned_adj/%s_PAGCL_0.050000_adj.pkl' % args.dataset, 'wb'))
...
if __name__ == '__main__':
    ...
    # data_polblogs_deep = Dataset(root='A:/pycharm_project/PAGCL-main/tmp/', name='polblogs')
    # data_polblogs_pyg = Dpr2Pyg(dpr_data=data_polblogs_deep, root='A:/pycharm_project/PAGCL-main/tmp/')

    model = Metacl(args, dataset, param, device).to(device)
    poisoned_adj = model.attack()
    pkl.dump(poisoned_adj.to(torch.device('cpu')), open('poisoned_adj/%s_PAGCL_0.100000_adj.pkl' % args.dataset, 'wb'))
